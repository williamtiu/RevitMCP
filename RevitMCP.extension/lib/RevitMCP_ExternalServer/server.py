...
try:
    ...
    def call_revit_listener(command_name: str, payload_data: dict = None):
        ...
        app.logger.info(f"Calling Revit Listener: POST to {revit_listener_direct_url} with payload {payload}")
        try:
            ...
            app.logger.info(f"Revit Listener success for {command_name}: {response_json}")
            return response_json # This will now be the direct return value for our MCP tools
        except requests.exceptions.ConnectionError:
            msg = f"Could not connect to the Revit Listener for command {command_name}."
            app.logger.error(msg)
            return {"status": "error", "message": msg}
        except requests.exceptions.Timeout:
            msg = f"Request to Revit Listener for command {command_name} timed out."
            app.logger.error(msg)
            return {"status": "error", "message": msg}
        except requests.exceptions.RequestException as e_req:
            msg_prefix = f"Error communicating with Revit Listener for {command_name}"
            if hasattr(e_req, 'response') and e_req.response is not None:
                ...
                try:
                    ...
                    full_msg = f"{msg_prefix}: HTTP {status_code}. Listener: {listener_err_data.get('message', 'Unknown listener error')}"
                    app.logger.error(full_msg, exc_info=True)
                ...
                app.logger.error(f"{msg_prefix}: {e_req}", exc_info=True)
                return {"status": "error", "message": f"{msg_prefix}: {e_req}"}
        except Exception as e_gen:
            app.logger.error(f"Unexpected error in call_revit_listener for {command_name}: {e_gen}", exc_info=True)
            return {"status": "error", "message": f"Unexpected error processing listener response for {command_name}."}
    ...
    def get_revit_project_info_mcp_tool() -> dict:
        ...
        return call_revit_listener(REVIT_INFO_TOOL_NAME)
    ...
    def get_revit_view_mcp_tool(view_name: str) -> dict:
        ...
        # Note: The listener command for fetching a view is "export_revit_view"
        return call_revit_listener("export_revit_view", {"view_name": view_name})
    ...
    SELECT_ELEMENTS_TOOL_DESCRIPTION_FOR_LLM = "Selects one or more elements in the current Revit model using their Element IDs. Use this if the user explicitly asks to select elements and provides their IDs."
    SELECT_ELEMENTS_TOOL_PARAMETERS_FOR_LLM = {
        "type": "object", "properties": {"element_ids": {"type": "array", "items": {"type": "string"}, "description": "An array of Element IDs (as strings) of the Revit elements to be selected. Can also be a single ID string if only one element is to be selected."}}, "required": ["element_ids"]
    }
            ...
            # --- Google Gemini Models ---
                ...
                if candidate.content.parts and candidate.content.parts[0].function_call:
                    ...
                    if function_name == REVIT_INFO_TOOL_NAME:
                        ...
                        # Gemini's args might already be list if schema specified array
                        ids_arg = function_args.get("element_ids", [])
                        if isinstance(ids_arg, str): ids_arg = [ids_arg] # ensure list
                        tool_result_data = select_elements_by_id_mcp_tool(element_ids=ids_arg)
                    else:
